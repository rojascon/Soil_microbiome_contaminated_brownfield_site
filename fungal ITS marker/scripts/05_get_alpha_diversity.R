...
##CODE FOR: 
#A) subsampling samples to 370 sequences each using mothur software 
...
################################################################################
#             1. Prepara data for subsampling to 370 sequences    
...
#370 appears like a good cutoff (only losing 4 samples)
...
write.table(mothur, file="fungal ITS marker/data/05_input_mothur.txt", 
            row.names=F, sep="\t");
...
#           sub.sample(shared=05_input_mothur.txt, size=370, persample=true)
#step4: rename output file as "05_output_mothur.txt") and place in the data directory

#PASTE Output from mothur here
# FUN_ITS_1_11_S11_L001 contains 202. Eliminating.
# FUN_ITS_1_48_S48_L001 contains 339. Eliminating.
# FUN_ITS_1_55_S55_L001 contains 260. Eliminating.
# FUN_ITS_1_6_S6_L001 contains 212. Eliminating.
...
mothur2=read.table("fungal ITS marker/data/05_output_mothur.txt", sep="\t",header=T);
...
################################################################################
#             4. Calculate soil microbiome alpha-diversity using phyloseq
################################################################################
...
################################################################################
save(alpha_meta, file="fungal ITS marker/data/05_alpha_diversity.Rdata");
