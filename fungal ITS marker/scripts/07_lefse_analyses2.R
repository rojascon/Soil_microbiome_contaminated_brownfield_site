...
################################################################################
#depth [all samples, do not include neighbor lot]
breaks=c(0.15, 1.4,4,7);
meta$depth2=cut(meta$sample_depth,
                breaks=breaks,
                include.lowest=TRUE,
                labels = c("0.15","1.5","4.60-6"));
...
write.table(dfA, "fungal ITS marker/data/07_depth_phylum_lefse.txt", 
            col.names=NA,row.names=TRUE,sep="\t");

write.table(dfB, "fungal ITS marker/data/07_depth_order_lefse.txt", 
            col.names=NA,row.names=TRUE,sep="\t");

write.table(dfC, "fungal ITS marker/data/07_depth_genus_lefse.txt", 
            col.names=NA,row.names=TRUE,sep="\t");
