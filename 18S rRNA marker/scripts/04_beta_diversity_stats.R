...
cca1=ggplot(cm, aes(CCA1, CCA2))+
    ...
    annotate(geom="text", x=-1.5, y=-0.4, label="Cobalt", size=3.5)+
    annotate(geom="text", x=-1.2, y=2, label="Chromium",size=3.5)+
    annotate(geom="text", x=-1.4, y=-1, label="Lead",size=3.5)+
    annotate(geom="text", x=3, y=0.2, label="Depth",size=3.5)+
    #add the biplot arrows (specify xend and yend coordinates)
    geom_segment(aes(x = 0, y = 0, xend =-1.1, yend = -0.4),
                 arrow = arrow(length = unit(0.3, "cm")), size=0.2)+
    geom_segment(aes(x = 0, y = 0, xend =-1, yend = 1.7),
                 arrow = arrow(length = unit(0.3, "cm")), size=0.2)+
    geom_segment(aes(x = 0, y = 0, xend =-1, yend = -1),
                 arrow = arrow(length = unit(0.3, "cm")), size=0.2)+
    geom_segment(aes(x = 0, y = 0, xend =2.7, yend = 0.2),
                 arrow = arrow(length = unit(0.3, "cm")), size=0.2)+
