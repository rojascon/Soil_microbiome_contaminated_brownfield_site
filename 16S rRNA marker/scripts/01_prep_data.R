...
asv=read.table("16S rRNA marker/data/00_brownfield_16S_ASVtable.txt", 
               header=T, sep="\t", row.names=1);
tax=read.table("16S rRNA marker/data/00_brownfield_16S_taxonomy.txt", 
               header=T, sep="\t");
meta=read.table("16S rRNA marker/data/00_brownfield_16S_metadata.txt", 
                header=T, sep="\t");
...
mytax=tax[,c(1:7)];
rownames(mytax)=mytax$X16S_seq_number; mytax$X16S_seq_number=NULL;
save(mytax, file="16S rRNA marker/data/01_brownfield_16S_taxonomy_reduced.Rdata");

#remove ASVs that are unclassified, eukarya, chloroplast, or mitochondria 
#from 16S ASV table
...
save(asvdf, file="16S rRNA marker/data/01_brownfield_16S_ASVtable_filtered.Rdata");
...
save(meta, file="16S rRNA marker/data/01_brownfield_16S_metadata_formatted.Rdata");
