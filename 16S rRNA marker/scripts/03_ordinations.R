...
#this asv table has no singleton or doubletons to remove
...
pcoa_met$sample_depth2=factor(pcoa_met$sample_depth2, 
                              levels=c("0.15", "1.52","3.05","4.6","6.1",
                                       "neighbor_lot"))
...
pcoa1=ggplot(pcoa_met, aes(Axis1, Axis2))+
  ...
  labs(y=paste("PC2 (",ax2,"%)",sep=""),
       x=paste("PC1 (",ax1,"%)",sep=""),
       fill="Depth (m)",
       title="16S rRNA marker (all samples)")+
...
pcoa2=ggplot(pcoa_met2, aes(Axis1, Axis2))+
  ...
  labs(y=paste("PC2 (",ax2,"%)",sep=""),
       x=paste("PC1 (",ax1,"%)",sep=""),
       fill="",
       title="16S rRNA marker (surface samples)")+
